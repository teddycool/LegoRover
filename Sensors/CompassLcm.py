...
import lcm
from LCM.compass import compass
...
class Compass(object):
    ...
    def update(self, kwarg):
        print "Compass update started...   "
        print "Endless update-loop in new thread"
        while 1:
            print "Compass update...   "
            self._x_out = (self._read_word_2c(3) + config["Compass"]["OffsetX"]) * config["Compass"]["Scale"]
            self._y_out = (self._read_word_2c(7) + config["Compass"]["OffsetY"]) * config["Compass"]["Scale"]
            self._z_out = self._read_word_2c(5) * config["Compass"]["Scale"]
            self._compass = (round(self._x_out, 2), round(self._y_out, 2), round(self._z_out, 2)) #List?
            ...
            #Fill msg
            self._msg.compass = self._compass
            self._msg.bearing = self._bearing

            print "Bearing: " + str(self._toDegrees(self._bearing))
            self._lc.publish(compass._channelName, compass._msg.encode())
            print "Compass: " + range._channelName
            time.sleep(0.5)
