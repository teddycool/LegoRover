...
try:
    ...
    config = {"PushButton": {"Pressed": 0.1, "LongPressed": 1.5}}
import time
import cv2
    ...
    def update(self):
        ...
        if self._gpio.input(self._inputpin) == False: #Button pressed, PullUp is released
            ...
            if self._lastpress != self._pressed: #First round
                ...
                if time.time() - self._presstime > config["PushButton"]["LongPressed"]:
                    self._state = self._states[2]
                else:
                    if time.time() - self._presstime > config["PushButton"]["Pressed"]:
